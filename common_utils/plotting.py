...
def generate_diverse_colors(n_colors, n_global_moves=32, ref_color=None):
    ...
    class Color:
        max_weighted_square_distance = (
            (((512 + 127) * 65025) >> 8) + 4 * 65025 + (((767 - 127) * 65025) >> 8)
        )
    ...
    colors = list(
        map(
            lambda h: Color(*tuple(int(h[i : i + 2], 16) for i in (0, 2, 4))),
            colors_hex,
        )
    )
            ...
            for index in range(starting_idx, len(colors)):
                ...
                for step in steps:
                    ...
                    if new_color.is_valid():
                        new_min_wsd = new_color.min_weighted_square_distance(
                            index, colors
                        )
